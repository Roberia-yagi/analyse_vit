...
def _default_input_root() -> Path:
    return (Path(__file__).resolve().parents[2] / "results" / "selected" / "angles").resolve()
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Extract features for all images under selected/angles directories.",
    )
    parser.add_argument(
        "--input-root",
        ...
        help=(
            ...
            "(default: repo_root/results/selected/angles)."
        ),
    )
    parser.add_argument(
        "--output-root",
...
def main() -> None:
    ...
    if input_root.name != "angles" or input_root.parent.name != "selected":
        raise SystemExit(
            "Input root must be the selected/angles directory "
            "(e.g. /.../results/selected/angles)."
        )
    ...
    if output_root != input_root:
        raise SystemExit(
            "Output root must match input root. Features are saved under selected/angles/*/features."
        )
