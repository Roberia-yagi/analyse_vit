...
LOGGER = logging.getLogger("analyse_vit.feature_extract_angles")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Extract features for all images under selected/angles/{without_composite,with_composite} "
            "directories."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Root directory that contains angle directories "
            "(default: repo_root/results/selected/angles/without_composite)."
        ),
    )
    parser.add_argument(
        "--output-root",
...
def main() -> None:
    ...
    if (
        input_root.name not in COMPOSITE_SUBDIRS
        or input_root.parent.name != "angles"
        or input_root.parent.parent.name != "selected"
    ):
        raise SystemExit(
            "Input root must be the selected/angles/{without_composite,with_composite} directory "
            "(e.g. /.../results/selected/angles/without_composite)."
        )
    ...
    if output_root != input_root:
        raise SystemExit(
            ...
            "selected/angles/{without_composite,with_composite}/*/features."
        )
    ...
    if not image_items:
        raise SystemExit(f"No images found under angle directories in {input_root}")
