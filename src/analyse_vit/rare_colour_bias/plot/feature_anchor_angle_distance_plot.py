...
LOGGER = logging.getLogger("analyse_vit.feature_anchor_angle_distance_plot")

DEFAULT_VISION_ORDER = ("pe-core-l14-336", "siglip2-giant-opt-patch16-384", "qwen3-vl-8b-embed")
DEFAULT_ANGLE_ORDER = ("front", "back", "left", "right", "top")
GEN_MODEL = "qwen"
...
def _default_output_root(selected_root: Path) -> Path:
    return (selected_root / "analysis" / "anchor_angle_distance").resolve()
...
def _list_angles(angles_root: Path, animals: Sequence[str]) -> List[str]:
    angles: set[str] = set()
    for animal in animals:
        animal_dir = angles_root / animal
        if not animal_dir.is_dir():
            continue
        for angle_dir in animal_dir.iterdir():
            if angle_dir.is_dir():
                angles.add(angle_dir.name)
    angles.add("front")
    return _ordered(sorted(angles), DEFAULT_ANGLE_ORDER)
...
def _collect_metrics(feature_dir: Path, anchor_center: torch.Tensor) -> Dict[str, List[float]]:
...
def _summarize(metrics: Dict[str, Dict[str, Dict[str, List[float]]]]) -> Dict[str, Dict[str, Dict[str, Dict[str, float]]]]:
    ...
    for metric_name, angle_map in metrics.items():
        summary[metric_name] = {}
        for angle, animal_map in angle_map.items():
            summary[metric_name][angle] = {}
            for animal, values in animal_map.items():
                mean_value = _mean(values)
                summary[metric_name][angle][animal] = {
                    "count": len(values),
                    "mean": mean_value,
...
def _plot_metric_grid(
    *,
    ...
    for angle in angles_sorted:
        ...
        color = angle_to_color.get(angle, "black")
        means = []
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot feature change metrics from anchor (front) center to each angle for each animal.",
    )
    parser.add_argument(
        "--selected-root",
        default=str(_default_selected_root()),
        help="Structured root with anchors/angles (default: repo_root/results/selected).",
    )
    parser.add_argument(
        "--gen-model",
        default=GEN_MODEL,
        help="Generation model (only qwen is supported).",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Angles to include (default: front + discovered angles).",
    )
    parser.add_argument(
        "--output-root",
        default=None,
...
def _run_structured(
    ...
    for vision in vision_models:
        for pooling in poolings:
            ...
            for animal in animals:
                ...
                for angle in angles:
                    ...
                    if not feature_dir.is_dir():
                        continue
                    metrics = _collect_metrics(feature_dir, center)
                    if not metrics["distance"]:
                        continue
                    for metric_key, values in metrics.items():
                        distances[metric_key].setdefault(angle, {})[animal] = values
                    added = True
            ...
            LOGGER.info("Saved plot: %s", out_dir / "anchor_angle_cosine_grid.png")
            LOGGER.info("Saved plot: %s", out_dir / "anchor_angle_relative_change_grid.png")
...
def main() -> None:
    ...
    if args.gen_model != GEN_MODEL:
        raise SystemExit("Only gen-model 'qwen' is supported for angle plots.")
    ...
    if args.angles:
        ...
        if "front" not in angles:
            raise SystemExit(
                "Angles must include 'front' to use selected/anchors/without_composite as the reference."
            )
    else:
        angles = _list_angles(selected_root / "angles" / COMPOSITE_SUBDIR, animals)
    if not angles:
        raise SystemExit("No angles found under selected/angles/without_composite.")

    if args.vision_model:
        vision_models = [args.vision_model]
    else:
        vision_models = _list_vision_models(anchors_root, animals, GEN_MODEL)
