...
Notes:
  - job.slurm must source race_guard.sh early (before the main workload).
...
Examples:
  sbatch_race_watch --gpu-ram 80 -- --gres=gpu:1 -J train job.slurm
  sbatch_race_watch --gpu-ram 40 -i 5 -n 50 -- -J exp job.slurm
USAGE
}
